...
def generate_pdg(code):
    ...
    pdg = get_data_flow(tmp_file.name, benchmarks=benchmarks, store_pdgs=None, save_path_pdg=False,
                         beautiful_print=False, check_json=False)
    # => equivalent to: pdg = get_pdg(file_path=tmp_file.name, res_dict=benchmarks)
