...
# TODO: improve efficiency by replacing all recursions with loops !!!!!
...
def add_missing_data_flow_edges(pdg: Node,
                                add_basic_df_edges: bool = False,
                                add_call_expr_df_edges: bool = False, # todo: use at all calls where add_my_data_flows= is used !!!
                                # => call expr DF edges rely on basic DF edges!!!
                                add_func_return_df_edges: bool = False, # todo: use at all calls where add_my_data_flows= is used !!!
                                # => func return DF edges rely on basic DF edges as well as assignment DF edges!!!
                                benchmarks: Optional[dict] = None) -> int:
    """
    ...
    In fact, you do not need to use DoubleX's PDG generation *at all* when using this!!!
    Besides, DoubleX generates some incorrect data flows as well which this function avoids...
    ...
    Parameters:
        ...
        add_call_expr_df_edges: whether to add data flow edges from call expression parameters to function/method
                                declaration parameters;
                                supply False if you intend to generate those lazily
                                (default; much better performance for larger, obfuscated extensions)
        add_func_return_df_edges: whether to add data flow edges from return statements inside declared functions to
    ...
    total_data_flow_edges_added: int = 0

    start_total = timeit.default_timer()

    # No. | Order      | Function                                                       | Example (x-->y)                  | Relies on basic DF edges | Relies on assignment DF edges | Relies on possible other DF edges
    # ----|------------|----------------------------------------------------------------|----------------------------------|--------------------------|-------------------------------|----------------------------------
    # 1   | I (lazy)   | add_basic_data_flow_edges()                                    | x=1; x;                          | no                       | no                            | no
    # 2   | -          | add_missing_data_flow_edges_function_parameters()              | function f(x) {x}                | no                       | no                            | no
    # 3   | 1          | add_missing_data_flow_edges_declarations_and_assignments_RTL() | y=x;                             | no                       | no                            | no
    # 4   | 2          | add_missing_data_flow_edges_standard_library_functions()       | Object.assign(y,x)               | no                       | no                            | no
    # 5   | 2          | add_missing_data_flow_edges_chrome_apis()                      | chrome.scripting.executeScript() | no                       | no                            | no
    # 6   | 2          | add_missing_data_flow_edges_IIFE_returns()                     | y = function() {return x}()      | no                       | no                            | no
    # 7   | 2          | add_missing_data_flow_edges_IIFEs()                            | !function(y){}(x)                | no                       | no                            | no
    # 8   | 3          | add_missing_data_flow_edges_dot_function_expression()          | x.then((y) => {})                | YES (uses Func)*         | YES (uses Func)*              | YES (uses Func)*
    # 9   | 3          | add_missing_data_flow_edges_declarations_and_assignments_LTR() | x.onsuccess = (y) => {};         | YES (uses Func)*         | YES (uses Func)*              | YES (uses Func)*
    # 10  | II (lazy)  | add_missing_data_flow_edges_call_expressions()                 | function f(y) {} f(x)            | YES (uses Func)          | YES (uses Func)               | YES (uses Func)
    # 11  | III (lazy) | add_missing_data_flow_edges_function_returns()                 | function f() {return x} y = f()  | YES                      | YES                           | YES

    # Notes on laziness:
    #   I: add_basic_data_flow_edges() must be lazy because otherwise it's O(n^2) where n = the total no. of Identifier
    #      Nodes.
    #   II: add_missing_data_flow_edges_call_expressions() must be lazy because it will call either Func()
    #       (which calls get_data_flow_parents_in_order_no_split()) or get_all_data_flow_ancestors() many times
    #       (for each CallExpression encountered), which will in turn trigger basic DF generation.
    #   III: add_missing_data_flow_edges_function_returns() must be lazy because it relies on basic DF edges (as well
    #        as all other DF edges) being present already.
    #   *: add_missing_data_flow_edges_dot_function_expression() and
    #      add_missing_data_flow_edges_declarations_and_assignments_LTR() should, in theory, also be lazy as they
    #      should rely on basic DF edges for resolving function identifiers. However, there will be so many instances
    #      of "x.foo(f)" and "x.foo = f" in code where f is *not* a reference to a function, that we will accept some
    #      possible missed or incorrect data flows there at the benefit of simplicity and speed.
    #      Note that you will still correctly find these data flows when using non-lazy DF generation!
    #      Also note that references to *declared* functions will *still* be found as
    #      function_Identifier_get_FunctionDeclaration() will still find those, even w/o basic data flow edges.
    #      Only references to functions that are not declared using FunctionDeclarations in the standard way will
    #      be missed/incorrect (e.g., a function declared using: "let f = (x) => 2*x;").
    ...
    if add_basic_df_edges:
        ...
        print(f"[Adding data flows] {edges_added_basic} basic data flow edges added between identifiers with the same "
              f"name.")
    ...
    # (3):
    start = timeit.default_timer()
    edges_added_declarations_and_assignments_RTL = add_missing_data_flow_edges_declarations_and_assignments_RTL(pdg)
    benchmarks["add_missing_data_flow_edges_declarations_and_assignments_RTL"] = timeit.default_timer() - start
    print(f"[Adding data flows] {edges_added_declarations_and_assignments_RTL} data flow edges added to declarations and "
          f"assignments (RTL/right-to-left flows).")
    total_data_flow_edges_added += edges_added_declarations_and_assignments_RTL
    ...
    # (6):
    start = timeit.default_timer()
    edges_added_IIFE_returns = add_missing_data_flow_edges_IIFE_returns(pdg)
    benchmarks["add_missing_data_flow_edges_IIFE_returns"] = timeit.default_timer() - start
    print(f"[Adding data flows] {edges_added_IIFE_returns} edges added to IIFE returns.")
    total_data_flow_edges_added += edges_added_IIFE_returns

    # (7):
    start = timeit.default_timer()
    edges_added_IIFEs = add_missing_data_flow_edges_IIFEs(pdg)
    benchmarks["edges_added_IIFEs"] = timeit.default_timer() - start
    print(f"[Adding data flows] {edges_added_IIFEs} edges added to IIFEs.")
    total_data_flow_edges_added += edges_added_IIFEs

    # (8):
    start = timeit.default_timer()
    edges_added_dot_function_expression = add_missing_data_flow_edges_dot_function_expression(pdg)
    benchmarks["add_missing_data_flow_edges_dot_function_expression"] = timeit.default_timer() - start
    print(f"[Adding data flows] {edges_added_dot_function_expression} edges added to x.foo((y) => ...) calls.")
    total_data_flow_edges_added += edges_added_dot_function_expression

    # (9):
    start = timeit.default_timer()
    edges_added_declarations_and_assignments_LTR = add_missing_data_flow_edges_declarations_and_assignments_LTR(pdg)
    benchmarks["add_missing_data_flow_edges_declarations_and_assignments_LTR"] = timeit.default_timer() - start
    print(f"[Adding data flows] {edges_added_declarations_and_assignments_LTR} edges added to declarations and "
          f"assignments (LTR/left-to-right flows).")
    total_data_flow_edges_added += edges_added_declarations_and_assignments_LTR

    # (10):
    if add_call_expr_df_edges:
        start = timeit.default_timer()
        ...
        identifiers1: List[Node] = [id1 for id1 in identifiers
                                    if
                                       ...
                                       (id1.is_within_the_nth_child_of_a(0, ["VariableDeclarator"]) or  # todo: more efficient function: add forbidden_ancestor_names param?
                                        ...
                                        id1.is_inside_or_is_any_function_declaration_param() or
...
def add_missing_data_flow_edges_declarations_and_assignments_RTL(pdg: Node) -> int:
    """
    Adds missing right-to-left (RTL) data flows from `y` to `x` in statements/expressions/declarations like:
    ...
    * ({x:x} = y);
    ...
    return data_flow_edges_added +\
        sum(add_missing_data_flow_edges_declarations_and_assignments_RTL(child) for child in pdg.children)


def add_missing_data_flow_edges_declarations_and_assignments_LTR(pdg: Node) -> int:
    """
    Adds left-to-right (LTR) data flows to assignments, namely from `x` to `y` in non-declaring member assignments where the
    RHS is an (Arrow)FunctionExpression; e.g.:

    * x.onsuccess = (y) => { /* ... */ };
    * x.onerror = (y) => { /* ... */ };

    NOTE:
    Uses Func() for statements like "x.onsuccess = some_identifier;" !!!
    Therefore, this method should be executed as late as possible !!!
    ...
    for assignment_expr in pdg.get_all_as_iter("AssignmentExpression"):
        # interface AssignmentExpression {
        #     operator: '=' | '*=' | '**=' | '/=' | '%=' | '+=' | '-=' |
        #               '<<=' | '>>=' | '>>>=' | '&=' | '^=' | '|=';
        #     left: Expression;
        #     right: Expression;
        # }
        if assignment_expr.attributes['operator'] == '=':
            # Note: We only care about functions being assigned to members, an assignment operator other than '='
            #       does not make any sense here!!!

            lhs = assignment_expr.lhs()
            rhs = assignment_expr.rhs()

            if lhs.name == "MemberExpression":
                leftmost_identifier = lhs.member_expression_get_leftmost_identifier()  # the "x" in "x.y" or "x[y]"

                if leftmost_identifier.name == "Identifier":  # could also be a "ThisExpression" but we're not handling that for now!
                    # Adds data flows in the left-to-right (LTR) direction, from `x` to `y` in non-declaring member
                    #     assignments where the RHS is an (Arrow)FunctionExpression; e.g.:
                    # x.onsuccess = (y) => { /* ... */ };
                    try:
                        func = Func(rhs, use_df_edges=False)
                        # => use_df_edges=False because this function is executed non-lazily!!!

                        # (1) Determine all function params:
                        func_params: List[Node] = func.get_params()
                        ...
                        # Do nothing because Identifier/Expression on the RHS may very well not refer to a function!
                        pass

    return data_flow_edges_added


def add_missing_data_flow_edges_call_expressions(pdg: Node,
                                                 identifier_of_interest_in: Optional[Node] = None, # todo: use
                                                 identifier_of_interest_out: Optional[Node] = None
                                                 ) -> int:
    """
    ...
    * foo.bar(y)
      // ...where 'foo' is an instance of class C and class C has a non-static method named 'bar' that takes a param 'x'

    * new C().bar(y)
      // ...where C is a class and class C has a non-static method named 'bar' that takes a param 'x'

    More formally: adds the following data flow edges:
        1. y --data--> x if - c is a CallExpression with an Identifier callee and at least 1 argument,
                            - f is the function to which the callee of c was resolved using Func(),
                            - x is the n-th param to the function f,
                            - and y is an identifier inside the n-th param passed to the call c:
                            function foo(x) {}; foo(y)
                            .........^^^....... ^^^...
                                      f          c
        2. y --data--> x if - c is a CallExpression with a MemberExpression callee (with the LHS l of the
                              MemberExpression either being an Identifier or a NewExpression and the RHS r either being
                              an Identifier with name r or, otherwise, statically evaluating to the string r)
                              and at least 1 argument,
                            - l has exactly 1 data flow ancestor d that is a ClassDeclaration,
                            - ClassDeclaration d has a non-static method m with the same name as r,
                            - x is an identifier inside the n-th param to the non-static method m,
                            - and y is an identifier inside the n-th param passed to the call c:
                            class C { r(x) {} } let l = C(); l.r(y)
                            ^^^^^^^^^^^^^^^^^^^              ^^^...
                                    d                         c

    NOTE:
    This function used Func() for resolving identifiers referencing functions to FunctionDeclarations!!!
    Therefore, this function should be executed as late as possible!!!

    NOTE:
    identifier_of_interest_in and identifier_of_interest_out shall not both be non-None!!!
    If so, identifier_of_interest_in being not None takes precedence.

    Parameters:
        pdg: the PDG (Program Dependence Graph), generated by DoubleX, to add the missing data flow edges to
        identifier_of_interest_in: when not None, only "_ --data--> identifier_of_interest_in" edges will be considered/
                                   generated, i.e., the Node whose *incoming* data flow edges you're interested in
        identifier_of_interest_out: when not None, only "identifier_of_interest_out --data--> _" edges will be considered/
                                    generated, i.e., the Node whose *outgoing* data flow edges you're interested in
    ...
    # ToDo: separate function for IIFEs not needed anymore?!

    if identifier_of_interest_in is not None:  # = the 'x' in: y --data--> x
        ancestor: Optional[Node] = identifier_of_interest_in.get_ancestor_or_none(["FunctionDeclaration",
                                                                                   "FunctionExpression",
        ...
        if ancestor is not None:
            ...
            #     expression: false;
            # }
            params: List[Node] = ancestor.get("params")
            param_idx: List[int] = [i for i, param in enumerate(params)
                                      if identifier_of_interest_in.is_inside_or_is(param)]
            if len(param_idx) == 0:
                return 0  # identifier_of_interest_in seems to be inside ancestor but not inside one of the params
            param_idx: int = param_idx[0]
            ...
            #     body: ClassBody;
            # }
            # ToDo: handle ClassExpressions, too!!!
            ...
            #     computed: boolean;
            #     value: FunctionExpression | null; <----- a FunctionExpression may be part of a MethodDefinition, or not!
            #     kind: 'method' | 'constructor';
            #     static: boolean;
            # }

            if ancestor.name in ["FunctionDeclaration", "ArrowFunctionExpression"] or ancestor.parent.name != "MethodDefinition":
                # 1.: y --data--> x=identifier_of_interest_in in:
                # function foo(x) {}; foo(y)
                # .........^^^....... ^^^...
                #           f /|\      c /|\
                #              |          |
                #          x=given    y=wanted
                func: Func = Func(ancestor)  # triggers no DF generation as ancestor in [FuncDecl, FuncExpr, ArrowFuncExpr]
                if id_node := func.get_id_node():  # "foo"  # ToDo: handle arguments.callee as well!!!
                    for df_descendent in id_node.get_all_data_flow_descendents(sort=True):  # c
                        if df_descendent.is_callee_of_a_call_expression():  # c()
                            call_expr: Node = df_descendent.parent
                            if param_idx < len(call_expr.get("arguments")):
                                # Add data flow edge:
                                for identifier in call_expr.get("arguments")[param_idx].get_all_identifiers():
                                    data_flow_edges_added += identifier.set_data_dependency(identifier_of_interest_in)

            else:  # if ancestor.name == "FunctionExpression" and ancestor.parent.name == "MethodDefinition":
                # 2.: y --data--> x=identifier_of_interest_in in:
                # class C { r(x) {} } let l = C(); l.r(y)
                # ^^^^^^^^^^^^^^^^^^^              ^^^...
                #         d  /|\                    c /|\
                #             |                        |
                #         x=given                  y=wanted
                method_definition: Node = ancestor.parent  # r(x) {}
                class_body: Node = method_definition.parent  # { r(x) {} }
                class_declaration: Node = class_body.parent  # class C { r(x) {} }
                if class_declaration.name == "ClassDeclaration":  # ToDo: handle ClassExpressions, too!!!
                    js_class: JSClass = JSClass(class_declaration)
                    id_: List[Node] = class_declaration.get("id")  # C
                    if len(id_) > 0:  # if declared class actually *has* a name C:
                        for df_descendent in id_[0].get_all_data_flow_descendents(sort=True):  # l
                            if (df_descendent.is_lhs_of_a("MemberExpression")  # l = "l" in "l.r"
                                    and df_descendent.parent.is_callee_of_a_call_expression()  # "l.r" is a callee
                                    and len(df_descendent.grandparent().get("arguments")) > 0):  # "l.r()" takes >=1 arg
                                # Name check: is the "r" found in "l.r" the same "r" as from the method definition that "x" is in?
                                callee_rhs: Node = df_descendent.get_sibling_relative(+1)  # the "r" found in "l.r"
                                class_method_name: str = callee_rhs.attributes['name'] \
                                    if callee_rhs.name == "Identifier" \
                                    else callee_rhs.try_static_eval(allow_partial_eval=False)
                                if [class_method_name] == [k.attributes['name'] for k in method_definition.get("key") if k.name == "Identifier"]:
                                    # Add a data flow from call to definition(=identifier_of_interest_in):
                                    call_arguments: List[Node] = df_descendent.grandparent().get("arguments")
                                    if len(call_arguments) < param_idx:
                                        for identifier in call_arguments[param_idx].get_all_identifiers():
                                            data_flow_edges_added += identifier.set_data_dependency(identifier_of_interest_in)

        return data_flow_edges_added

    for call_expr in pdg.get_all_as_iter("CallExpression"):
        # interface CallExpression {
        #     callee: Expression | Import;
        #     arguments: ArgumentListElement[];
        # }

        if identifier_of_interest_out is not None and not identifier_of_interest_out.is_inside(call_expr):
            continue

        if len(call_expr.children) > 1 and call_expr.children[0].name == "Identifier":
            ...
            callee: Node = call_expr.get('callee')[0]  # e.g., the "foo" in "foo(x)"

            try:
                func: Func = Func(callee)  # `function foo(x) { ... }`
                no_of_params_in_call = len(call_expr.get('arguments'))
                no_of_params_in_declaration = len(func.get_params())
                for param_index in range(min(no_of_params_in_call, no_of_params_in_declaration)):
                    n_th_call_param = call_expr.get('arguments')[param_index]
                    n_th_declaration_param = func.get_nth_param(n=param_index)
                    ...
                    if n_th_declaration_identifier is not None:  # ToDo: handle destructured params as well!!!
                        # Add a data flow edge for each identifier in the expression used as the parameter:
                        for identifier in n_th_call_param.get_all_identifiers(): # ToDo: use _not_inside_a() ?!?!?!
                            # Add data flow edge:
                ...
                #     * built-ins (e.g., parseInt())
                #     * imported from somewhere, or
                #     * created dynamically
                # ...and we can't find a declaration.
                if callee.name == "Identifier" and callee.attributes['name'] not in JAVASCRIPT_BUILT_IN_FUNCTIONS:  # ToDo: put logic somewhere else ?!
                    if os.environ.get('WARN_FUNC_DEF_NOT_FOUND') == "yes":
                        print(f"[Warning] declaration of function '{callee.attributes['name']}' not found (line "
                              f"{callee.get_line()}, file {callee.get_file()}), "
        ...
        elif len(call_expr.children) > 1 and call_expr.children[0].name == "MemberExpression":
            ...
            #     [...]
            #     ...
            callee_member_expression: Node = call_expr.children[0]
            ...
            callee_rhs: Node = callee_member_expression.rhs()  # an Identifier, or an Expression (e.g.: foo['b'+'ar'])
            ...
            if callee_lhs.name in ["Identifier", "NewExpression"]:
                ...
                # We need to check *again* if callee_lhs is an Identifier as the "callee" of a NewExpression need not be
                #   a simple Identifier, e.g.: "new Foo.Bar()":
                if callee_lhs.name == "Identifier":
                    arguments: List[Node] = call_expr.children[1:]
                    assert len(arguments) > 0  # because of len(call_expr.children) > 1 check above
                    ...
                    class_method_name: str = callee_rhs.attributes['name']\
                                             if callee_rhs.name == "Identifier"\
                                             else callee_rhs.try_static_eval(allow_partial_eval=False)  # "bar"

                    if isinstance(class_method_name, str):
                        ...
                        data_flow_class_decl_ancestors: Set[Node] =\
                            {
                                ...
                                if df_ancestor.parent.name == "ClassDeclaration"  # ToDo: handle ClassExpressions, too!
                        ...
                        if len(data_flow_class_decl_ancestors) == 1:
                            ...
                            print(f"[Warning] found {len(data_flow_class_decl_ancestors)} possible ClassDeclarations "
                                  f"the LHS identifier of the CallExpression in line {call_expr.get_line()} "
                                  f"(file {call_expr.get_file()}) could refer to; not adding any data flows.")
...
def add_missing_data_flow_edges_IIFEs(pdg: Node) -> int:
    """
    Handles data flows in IIFEs (Immediately Invoked Function Expressions) like from `a` to `x` in this one:
        !function(x,y){}(a,b)
    """
    data_flow_edges_added: int = 0

    if (pdg.name == "CallExpression"
        ...
        no_of_params_in_call_expr: int = len(
            pdg.children) - 1  # a CallExpression = 1 callee + N arguments (cf. Esprima docs)
    ...
    return data_flow_edges_added + \
        sum(add_missing_data_flow_edges_IIFEs(child) for child in pdg.children)


def add_missing_data_flow_edges_dot_function_expression(pdg: Node) -> int:
    """
    Adds missing data flows from `y` to `x` in CallExpressions like:
    ...
    * y.transaction("accounts").objectStore("accounts").get("Alice").addEventListener("success", (x) => {});
    ...
    for call_expr in pdg.get_all_as_iter("CallExpression"):
        ...
        if callee.name == "MemberExpression" and len(arguments) > 0:
            # CallExpressions where the callee is a MemberExpression, like `foo.bar(...)`:
            # [CallExpression]
            #     [MemberExpression:"False"]
            #         [Identifier/MemberExpression]
            #         [Identifier]
            #     [...]
            #     ...
            callee_member_expression: Node = callee
            assert callee_member_expression.name == "MemberExpression"  # because of check above
            callee_lhs: Node = callee_member_expression.lhs()  # an Identifier, NewExpression, ... or another MemberExpression
            callee_leftmost_identifier: Node = callee_member_expression.member_expression_get_leftmost_identifier()
            callee_rhs: Node = callee_member_expression.rhs()  # an Identifier, or an Expression (e.g.: foo['b'+'ar'])
            ...
            if callee_leftmost_identifier.name == "Identifier":
                ...
                for argument in arguments:  # for each "([a,b], [c,d]) => console.log(a,b,c,d)":
                    try:
                        func: Func = Func(argument, use_df_edges=False)
                        # => use_df_edges=False because this function is executed non-lazily!!!

                        for func_param in func.get_params():  # for each "[a,b]":
                            for identifier in func_param.function_param_get_identifiers():   # for each "a":
                                data_flow_edges_added += callee_leftmost_identifier.set_data_dependency(identifier)  # foo --data--> a
                    except FuncError:
                        pass
...
def add_missing_data_flow_edges_function_returns(pdg: Node,
                                                 identifier_of_interest_in: Optional[Node] = None,
                                                 identifier_of_interest_out: Optional[Node] = None) -> int:
    """
    ...
    You can see that this function relies on existing data flow edges, including basic data flow edges.
    Basic data flow edges, call expression data flow edges and other function return data flow edges # todo?!
    will be created lazily on demand, all others however need to have been generated in advance.
    Therefore, this function is executed last.

    If `identifier_of_interest_in` is not None, only data flow edges where y=identifier_of_interest_in
    will be generated.

    If `identifier_of_interest_out` is not None, only data flow edges where x=identifier_of_interest_out
    will be generated.

    Do not set both identifier_of_interest_in and identifier_of_interest_out to non-None values:
    The value of identifier_of_interest_in will be ignored when identifier_of_interest_out is not None.

    Cf. (B) add_missing_data_flow_edges_IIFE_returns(), which does the same for IIFEs.

    Parameters:
        pdg: the PDG over whose ReturnStatements this function will iterate when identifier_of_interest_out is None
             and identifier_of_interest_in is None
        identifier_of_interest_in: when not None, only "_ --data--> identifier_of_interest_in" edges will be considered/
                                   generated, i.e., the Node whose *incoming* data flow edges you're interested in
        identifier_of_interest_out: when not None, only "identifier_of_interest_out --data--> _" edges will be considered/
                                    generated, i.e., the Node whose *outgoing* data flow edges you're interested in
    ...
    if identifier_of_interest_out is None and identifier_of_interest_in is None:
        for return_statement in pdg.get_all_as_iter("ReturnStatement"):
            ...
            if len(argument) > 0:  # There's no data flow for a simple "return;" w/o any return value.
                ...
                # IMPORTANT NOTE: the Func() constructor call above does **NOT** require any DF edges to be present!!!!!
    ...
    elif identifier_of_interest_out is not None:  # Generate only "identifier_of_interest_out --data--> y" edges:
        if return_statement := identifier_of_interest_out.get_surrounding_return_statement():
            # x=identifier_of_interest_out (inside argument)
            f: Func = Func(return_statement.return_statement_get_function())
            # The line above shouldn't throw any Exception, therefore we're not catching any!
            # IMPORTANT NOTE: the Func() constructor call above does **NOT** require any DF edges to be present!!!!!
            ...
            if f.is_function_declaration() and f.get_id_node() is not None:
                ...
                for f_ in map(lambda c: c.extremity, f.get_id_node().data_dep_children()):
                    if f_.is_callee_of_a_call_expression():
                        for y in map(lambda c: c.extremity, f_.data_dep_children()):
                            data_flow_edges_added += identifier_of_interest_out.set_data_dependency(y)
                            # => includes call: x._data_dep_children.append(extremity=y) where x=identifier_of_interest_out

    elif identifier_of_interest_in is not None:  # Generate only "x --data--> identifier_of_interest_in" edges:
        for df_parent in map(lambda p: p.extremity, identifier_of_interest_in.data_dep_parents(lazy_gen_func_return_df=False)):  # f_
            if df_parent.is_callee_of_a_call_expression():
                for df_grandparent in map(lambda p: p.extremity, df_parent.data_dep_parents()):  # f
                    if df_grandparent.is_id_of_function_declaration():
                        func: Node = df_grandparent.parent
                        assert func.name == "FunctionDeclaration"
                        for return_statement in func.get_all_as_iter("ReturnStatement"):
                            # interface ReturnStatement {
                            #     argument: Expression | null;
                            # }
                            argument: List[Node] = return_statement.get("argument")
                            if len(argument) > 0:  # ignore "return;"
                                for x in argument[0].get_all_identifiers():
                                    data_flow_edges_added += x.set_data_dependency(identifier_of_interest_in)
                                    # => includes call: x._data_dep_children.append(extremity=y)

    return data_flow_edges_added


def add_missing_data_flow_edges_IIFE_returns(pdg: Node) -> int:  # TODO: handle destructuring assignments!!!
    """
    (B) Ensures there's a data from `a` to `b` in the following code (IIFEs):
    ```
    ...
    Cf. (A) add_missing_data_flow_edges_function_returns(), which does the same for declared functions.
    """
    data_flow_edges_added: int = 0
    ...
    for return_statement in pdg.get_all_as_iter("ReturnStatement"):
        ...
        if len(argument) > 0:  # There's no data flow for a simple "return;" w/o any return value.
            ...
            if f.is_immediately_invoked():  # (only (arrow) function expressions can be immediately invoked)
